...
Last modified: Wed Dec 11 15:19:13 2019
"""
...
def csv_ana(rpath,PCE):
    ...
    for cc in ccs:
        if ("APA3" in PCE):
            if (float(cc[1])*512 + float(cc[2])*128 + float(cc[5])) in range(2416, 2432, 1):
                flg = False
            else:
                flg = True
        elif ("APA4" in PCE):
            if (float(cc[1])*512 + float(cc[2])*128 + float(cc[5])) in [327]:
                flg = False
            else:
                flg = True
 
        if (float(cc[14]) > 80 ) and (float(cc[14]) < 180 ) and flg:
            chns.append(float(cc[1])*512 + float(cc[2])*128 + float(cc[5]))
            rmss.append(float(cc[8]))
            peds.append(float(cc[9]))
            sfrms.append(float(cc[10]))
            sfpeds.append(float(cc[11]))
            unstk_ratios.append(float(cc[12]))
            fgains.append(float(cc[14]))
            fg_inls.append(float(cc[15]))
            fareagains.append(float(cc[16]))
            fareag_inls.append(float(cc[17]))
 
        else:
            bchns.append(float(cc[1])*512 + float(cc[2])*128 + float(cc[5]))
            brmss.append(float(cc[8]))
            bpeds.append(float(cc[9]))
            bsfrms.append(float(cc[10]))
            bsfpeds.append(float(cc[11]))
            bunstk_ratios.append(float(cc[12]))
            bfgains.append(float(cc[14]))
            bfg_inls.append(float(cc[15]))
            bfareagains.append(float(cc[16]))
            bfareag_inls.append(float(cc[17]))
            print ("BAD", cc)
 
    print (len(fgains), np.mean(fgains), np.std(fgains), np.mean(fareagains), np.std(fareagains), np.mean(rmss), np.std(rmss))
    return ( (chns, rmss, peds, sfrms, sfpeds, unstk_ratios, fgains, fg_inls, fareagains, fareag_inls), 
             (bchns, brmss, bpeds, bsfrms, bsfpeds, bunstk_ratios, bfgains, bfg_inls, bfareagains, bfareag_inls) )

 

rpath = "/Users/shanshangao/Documents/tmp/PD4/"
#t_pat = "Test018"
#t_pat = sys.argv[1]
PCE = "APA3_09_23_2018_run03rms_run02fpg_run01asi.csv" 
#PCE = "APA4_09_23_2018_run03rms_run02fpg_run01asi.csv" 
old_results = csv_ana(rpath,PCE)
PCE = "APA3_11_06_2019_run01rms_run01fpg_run01asi.csv" 
#PCE = "APA4_12_05_2019_run01rms_run01fpg_run01asi.csv" 
new_results = csv_ana(rpath,PCE)
...
if (True):
    ...
    plt.scatter(old_results[0][0], old_results[0][6], label =PCE[0:4] + ": 09/23/2018")
    if ("APA3" in PCE[0:4] ):
        plt.scatter(new_results[0][0], new_results[0][6], label =PCE[0:4] + ": 11/06/2019")
    else:
        plt.scatter(new_results[0][0], new_results[0][6], label =PCE[0:4] + ": 12/05/2019")
    plt.title(PCE[0:4] + ": Inverted Gain Distribution (e-/LSB)" )
    plt.ylim((100, 160))
    plt.xlim((-50, 2650))
    plt.xlabel("APA Channel Number")
    plt.ylabel("Inverted Gain (e-/LSB)")
    plt.grid()
    plt.legend()
    plt.tight_layout()
    plt.savefig(rpath + PCE[0:4] + "_gain_dist" + ".png" )
    plt.close()
#    plt.show()
...
if (True):
    plt.rcParams.update({'font.size': 18})
    fig = plt.figure(figsize=(12,4))
    plt.hist(old_results[0][6],bins=60, range=(100,160), histtype='bar', label =PCE[0:4] + ": 09/23/2018, %d CHs, mean = %.5f e-/LSB, std = %.6f"%(len(old_results[0][6]), np.mean(old_results[0][6]), np.std(old_results[0][6])) )
    if ("APA3" in PCE[0:4] ):
        plt.hist(new_results[0][6],bins=60, range=(100,160), histtype='bar', label =PCE[0:4] + ": 11/06/2019, %d CHs, mean = %.5f e-/LSB, std = %.6f"%(len(new_results[0][6]), np.mean(new_results[0][6]), np.std(new_results[0][6])) )
    else:
        plt.hist(new_results[0][6],bins=60, range=(100,160), histtype='bar', label =PCE[0:4] + ": 12/05/2019, %d CHs, mean = %.5f e-/LSB, std = %.6f"%(len(new_results[0][6]), np.mean(new_results[0][6]), np.std(new_results[0][6])) )
    plt.title(PCE[0:4] + ": Histogram of Inverted Gain")
    plt.ylim((0, 300))
    plt.xlim((100, 160))
    plt.xlabel("Inverted Gain / (e-/LSB)")
    plt.ylabel("Channel Count")
    plt.grid()
    plt.legend(fontsize=14)
    plt.tight_layout()
    plt.savefig(rpath + PCE[0:4] + "_Hist_gain" + ".png" )
    #plt.show()
    plt.close()
...
if (True):
    ...
    #ax1 = fig.add_subplot(121)
    #ax2 = fig.add_subplot(122)
    plt.scatter(old_results[0][0], old_results[0][8], label =PCE[0:4] + ": 09/23/2018")
    if ("APA3" in PCE[0:4] ):
        plt.scatter(new_results[0][0], new_results[0][8], label =PCE[0:4] + ": 11/06/2019")
    else:
        plt.scatter(new_results[0][0], new_results[0][8], label =PCE[0:4] + ": 12/05/2019")
    plt.title(PCE[0:4] + ": Inverted Area Gain Distribution (unit: e-/(LSB)*(6$\mu$s) )" )
    plt.ylim((15, 30))
    plt.xlim((-50, 2650))
    plt.xlabel("APA Channel Number")
    #plt.ylabel("Inverted Area Gain \n e-/(LSB)*(6$\mu$s)")
    plt.ylabel("Inverted Area Gain")
    plt.grid()
    plt.legend()
    plt.tight_layout()
    plt.savefig(rpath + PCE[0:4] + "_areagain_dist" + ".png" )
    plt.close()
    #plt.show()
...
if (True):
    plt.rcParams.update({'font.size': 18})
    fig = plt.figure(figsize=(12,4))
    plt.hist(old_results[0][8],histtype='bar', bins=40, range=(20,30), label =PCE[0:4] + ": 09/23/2018, %d CHs, mean = %.5f e-/(LSB)*(6$\mu$s), std = %.6f"%(len(old_results[0][8]), np.mean(old_results[0][8]), np.std(old_results[0][8])) )
    if ("APA3" in PCE[0:4] ):
        plt.hist(new_results[0][8],histtype='bar', bins=40, range=(20,30), label =PCE[0:4] + ": 11/06/2019, %d CHs, mean = %.5f e-/(LSB)*(6$\mu$s), std = %.6f"%(len(new_results[0][8]), np.mean(new_results[0][8]), np.std(new_results[0][8])) )
    else:
        plt.hist(new_results[0][8],histtype='bar', bins=40, range=(20,30), label =PCE[0:4] + ": 12/05/2019, %d CHs, mean = %.5f e-/(LSB)*(6$\mu$s), std = %.6f"%(len(new_results[0][8]), np.mean(new_results[0][8]), np.std(new_results[0][8])) )
    plt.title(PCE[0:4] + ": Histogram of Inverted Area Gain")
    plt.ylim((0, 300))
    plt.xlim((20, 30))
    plt.xlabel("Inverted Gain / (e-/(LSB)*(6$\mu$s)")
    plt.ylabel("Channel Count")
    plt.grid()
    plt.legend(fontsize=14)
    plt.tight_layout()
    plt.savefig(rpath + PCE[0:4] + "_Hist_areagain" + ".png" )
    plt.close()
    #plt.show()

#

#print (len(fgains), np.mean(fgains), np.std(fgains), np.mean(rmss), np.std(rmss))
#print (bchns)
